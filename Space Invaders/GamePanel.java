...
public class GamePanel extends JPanel implements ActionListener, KeyListener {
        ...
        for (int i = enemies.size() - 1; i >= 0; i --) {
            if (enemies.get(i).isShooting()) { // if the enemy is shooting
                if (enemies.get(i).getBullet().getHitbox().intersects(tank.getHitbox())) { // if the enemy's bullet hits the tank
                    ...
                    tank.setLives(tank.getLives() - 1); // remove a life from the tank
                }
            }
        }
    }
    ...
    public void updateHighScoreFile() { // updates the high score file
        try {
            ...
            high_score = Integer.toString(Math.max(Integer.parseInt(high_score), score)); // set the high score to the max of the current high score and the current score
            outFile.print(high_score); // write the high score to the file
            outFile.close();
        }
        catch (IOException e) {
