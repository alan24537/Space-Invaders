...
public class IntroPanel extends JPanel implements MouseListener, MouseMotionListener, ActionListener {
    ...
    private ArrayList<Enemy> enemies = new ArrayList<Enemy>(); // arraylist of enemies that are displayed on the screen
    ...
    public IntroPanel() { // constructor
        ...
        fillEnemyList(200, 0.001); // fills the enemy list with 55 enemies
    ...
    public void paint(Graphics g) { // paints the intro panel
        if (!inInstructions) { // if the instructions are not being displayed
        ...
        else { // if the instructions are being displayed
            g.setColor(Color.BLACK);
            g.fillRect(0, 0, getWidth(), getHeight());
            ...
            g.drawString("Use the arrow keys to move the player left and right", Util.centreText("Use the arrow keys to move the player left and right", SpaceInvaders.WIDTH, g.getFont()), 650);
            g.drawString("Use the space bar to shoot", Util.centreText("Use the space bar to shoot", SpaceInvaders.WIDTH, g.getFont()), 700);
            g.drawString("Use the escape key to pause the game", Util.centreText("Use the escape key to pause the game", SpaceInvaders.WIDTH, g.getFont()), 750);
            g.drawString("Press \"m\" to mute all sounds", Util.centreText("Press \"m\" to mute all sounds", SpaceInvaders.WIDTH, g.getFont()), 800);
    ...
    public void fillEnemyList(int y, double fire_rate) { // fills the enemy list with 55 aliens (5 rows of 11 aliens)
        for (int i = 0; i < 11; i ++) {
            enemies.add(new Enemy(0, 100 + 98 + (i * (38 + 30)), y, fire_rate));
        }
        for (int i = 0; i < 11; i ++) {
            enemies.add(new Enemy(1, 100 + 98 + (i * (51 + 16)), y + 70, fire_rate));
        }
        for (int i = 0; i < 11; i ++) {
            enemies.add(new Enemy(1, 100 + 94 + (i * (51 + 16)), y + 140, fire_rate));
        }
        for (int i = 0; i < 11; i ++) {
            enemies.add(new Enemy(2, 100 + 90 + (i * (56 + 12)), y + 205, fire_rate));
        }
        for (int i = 0; i < 11; i ++) {
            enemies.add(new Enemy(2, 100 + 90 + (i * (56 + 12)), y + 275, fire_rate));
        }
    }
